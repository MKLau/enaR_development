...
\citep{wasserman94, barabasi12, borgatti2003network,
  freeman2004development, newman03review}. However, it seems likely
...
estuary sediementary nitrogen cycle.  Further, scientists have used
...
% Software Objectives
\enaR\ is an open-source software to facillitate ENA.  Extant ENA
software \citep{ulanowicz91, kazanci07, allesina04_wand,
  christensen04, fath06} each have crtical limitations, which led us
...
every funciton in \R\ is available from within \R\ (e.g.,
...
use and interpretaiton is beyond the scope of this short paper, but we
...
% size of the package and its capabiilty.
...
generally be appled to any Input-Output system that follows a
thermodynamicaly conserved unit among the compartments.  Thus, it is a
...
%export, as well as some speciality analyses (Table~\ref{tab:alg2}).
...
visulization is included in the online supplimentary information.
...
of YY funcitons in the \enaR\ package.
...
characterizes ecosystem growth and development \citet{ulanowicz86,
  ulanowicz97}.  This work is often refered to as ``Ecological
...
%be in the current \R\ working directory (online suppliment).  
...
\texttt{pack} function to create the requried network data object.
...
steady-state, we could have then appled one of four automated
...
relationships among the ecosysetm components when we consider the
...
There are several features of the \enaR\ packagw beyond the core
analyses that add substantive value for users.  We highlight three of
these featrues here.
...
A fourth added benifit of the \enaR\ package design is that it enables
...
software and facillitate wide development and use. It does this by (1)
providing greater accessibility to the code (e.g., free and open
source software avialable on multiple OS), (2) collecting a broad set
...
which may make \enaR\ more suited to advanced practicioners.
...
In conclusion, \enaR\ is an \R\ pacakge to facilitate the use and the
...
is rapidly growing in part becuase the tools and techniques let
...
\begin{table*}[h]
\center
\caption{Selected \enaR auxillary functions and analyses.}\label{tab:algaux}
\tableline
%\begin{scriptsize}
\begin{tabular}{l p{9 cm} l }
  \textbf{Function} & \textbf{Description} &
  \textbf{Example Reference}\\ [1 ex] \hline \\ [-0.5 ex]
  \multicolumn{3}{l}{\textbf{Speciality Analyses} }\\
  ...
  %findPathLength & \\[1 ex]
  \multicolumn{3}{l}{\textbf{Auxillary Functions}} \\
  get.orient & Determine the oreintation of the results
  (row-to-column  vs.\ School)  \\
  set.orient & Set the oreintation of the results
  (row-to-column vs.\ School) \\
