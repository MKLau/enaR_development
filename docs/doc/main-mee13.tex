...
\section*{Abstract}
\begin{itemize}
\item Network analysis is useful for approaching complex, relational
  datasets in many biological fields, including ecology and molecular
  and evolutionary biology.
\item Ecological Network Analysis (ENA) is an analytical tool set
  rooted in ecosystem ecology with over 30 years of development that
  investigates the structure, function, and evolution of ecological
  systems. Here, we introduce \enaR , an \R\ package that enables
  ecologists to perform a broad set of ENA algorithms that analyze the
  structure and dynamics of matter or energy movement between discrete
  ecological compartments (e.g., food webs).
