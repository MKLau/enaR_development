...
\citet{finn1980} used ENA flow analysis to compare the cycling of
multiple nutrients through the Hubbard Brook Ecosystem, New Hampshire,
USA, and \citet{vanoevelen} used the technique to show how different
...
\cite{bonavalli99}.  This is in part because the alligators also eat
...
\subsubsection{Quickly Applying Sets Analyses}
...
\cite{brandes05, wasserman94}.  In general the goal is to describe the
...
network modeling and analysis software \cite{batagelj07}.  The package
...
\textit{enaR}. Programmers that are interested in contributing can
connect online via \url{github.com/SEELab/enaR_development}.
